        ...
        <div class="p-4 w-full overflow-x-auto">
            ...
            <div class="grid grid-cols-5 space-x-4 h-full overflow-hidden">
                        ...
                        <div v-for="field in collection.fields" :key="field.id">
                            ...
                            <div class="mt-1 relative">
                                <div v-if="field.type == 'text'">
                                    <input v-if="field.slug_field === undefined" type="text" v-model="newData.data[field.name]" :placeholder="field.placeholder" v-forminput="newData.errors['data.'+field.name]">
                                ...
                                <div v-if="field.type == 'email'">
                                    <div class="mt-1 flex rounded-sm shadow-sm">
                                        <span class="inline-flex items-center px-3 rounded-l-sm border border-r-0 border-gray-300 bg-gray-50 text-gray-500 text-sm">@</span>
                                ...
                                <div class="w-full xl:w-1/4" v-if="field.type == 'enumeration'">
                                    <v-select :options="field.options.enumeration" class="v-select" placeholder="Select" v-model="newData.data[field.name]"></v-select>
                                </div>
                                ...
                                <div v-if="field.type == 'date'" class="w-full xl:w-1/4">
                                    <v-date-picker v-model="newData.data[field.name]">
                ...
                <div class="col-span-2 xl:col-span-1 mt-2 ml-2">
                    ...
                    <div class="bg-white mb-2 rounded-sm">
                        <div class="p-4 text-sm text-gray-700">
                            <div class="flex justify-between mb-4">
                                ...
                                <div class="text-gray-400">{{ content.created_at | date('D MMM YYYY, H:mm') }}</div>
                            </div>
                            <div class="flex justify-between">
                                <div class="font-semibold">Last Updated</div>
                                <div class="text-gray-400">{{ content.updated_at | date('D MMM YYYY, H:mm') }}</div>
                            </div>
                            <div class="flex justify-between mt-4" v-if="content.published_at !== null">
                                <div class="font-semibold">Published</div>
                                <div class="text-gray-400">{{ content.published_at | date('D MMM YYYY, H:mm') }}</div>
                            </div>
                        </div>
                    </div>
                    <div class="bg-white mb-2 rounded-sm">
                    ...
                    <div class="bg-white mb-2 rounded-sm" v-if="content.published_at !== null">
                        <div class="bg-gray-200 text-gray-700 hover:bg-gray-300 hover:text-gray-800 items-center px-4 py-2 rounded-sm text-sm text-center focus:outline-none transition ease-in-out duration-150 block cursor-pointer" @click="unpublish()">
                            <i class="fa fa-pencil-ruler"></i> Mark as Draft
                        </div>
                    </div>
                    <div class="bg-white mb-2 rounded-sm">
                        <div class="bg-orange-500 items-center px-4 py-2 rounded-sm text-sm text-white text-center focus:outline-none transition ease-in-out duration-150 block hover:bg-orange-600 hover:text-white cursor-pointer" @click="moveToTrashContent()">
                            <i class="fa fa-trash-restore"></i> Move to Trash
                        </div>
                    </div>
                    <div class="bg-white mb-2 rounded-sm">
                        <div class="bg-red-500 items-center px-4 py-2 rounded-sm text-sm text-white text-center focus:outline-none transition ease-in-out duration-150 block hover:bg-red-600 hover:text-white cursor-pointer" @click="deleteContent()">
    ...
    methods: {
        ...
        clearNewData(){
            ...
            for (const $ref in this.$refs) {
                for (const [index, $component] in this.$refs[$ref]) {
                    if (this.$refs[$ref][index].hasOwnProperty('codemirror')) {
                        this.$refs[$ref][index].codemirror.refresh();
                        this.$refs[$ref][index].codemirror.setValue('');
                    }
                }
            }
        },
